...
public class _casaIt extends PortaleImmobiliare {     

    //Variabili generali
	private final String NOMEPORTALE = "casa.it";
	private final String SESSIONCOOKIENAME = "JSESSIONID";
	private final String SESSIONCOOKIEDOMAIN = "www.casa.it";
	private final String URLROOT = "http://www.casa.it";
	...
	//Costruttore
	public _casaIt (String urlIcona, String valoreLabel, String idPortale, boolean isActive) {		
    ...
    public boolean inserisciScheda(SchedaImmobile scheda, boolean isSequential) throws HttpCommunicationException {
    	...
    	try {
			connessione_0.get("Connessione 0 - GET della home page", URLROOT + "/vendita", debugMode);
		} catch (IOException e) {
    	...
    	try {
			...
			connessione_5.get("Connessione 5 - GET di una pagina per passargli il codice agenzia e riferimento annuncio", URLROOT + "/area_clienti/include/ajax.php?edit=valida_rif_agenzia&rif_agenzia=" + encodedSchedaCodice + "&codice_inserzione=&codice_cliente=" , debugMode);
		} catch (IOException e) {
			throw new HttpCommunicationException(e);
		}
    	...
    	for(int i=0; i<scheda.arrayImages.length; i++) {
    		if(scheda.arrayImages[i]!=null) {
    	        ...
    	        try {
    	        	Object[] response = connessione_6.post("Connessioni 6 - inserimento immagine " + i, URLROOT + "/area_clienti/include/upload_foto.php?i=" + i + "&codice_cliente=" +  reqEntity, debugMode);
    	        	String responseBody = (String)response[1];
